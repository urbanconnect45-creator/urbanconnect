...
set delivery_contact_phone = app_users.phone_number
from public.app_users
where orders.user_id = app_users.id
  and nullif(trim(coalesce(orders.delivery_contact_phone, '')), '') is null
  and nullif(trim(coalesce(app_users.phone_number, '')), '') is not null;

update public.delivery_jobs as jobs
set
  delivery_contact_phone = orders.delivery_contact_phone,
  updated_at = now()
from public.orders
where orders.id = jobs.order_id
  and nullif(trim(coalesce(jobs.delivery_contact_phone, '')), '') is null;
